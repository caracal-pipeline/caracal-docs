...
:ref:`flag: label <flag_label>` parameter in this worker.
...
* Flag on autocorrelations to catch antennas with obvious problems using the custom
  program POLITSIYAKAT (:ref:`flag: autoflag_autocorr_powerspectra <flag_autoflag_autocorr_powerspectra>`).
...
* Flag according to a static mask of bad frequency ranges using the custom program
  RFIMASKER (:ref:`flag: static_mask <flag_static_mask>`). The mask file should
...
* Flag with AOFlagger (:ref:`flag: autoflag_rfi <flag_autoflag_rfi>`). The
